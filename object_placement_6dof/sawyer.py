...
class sawyer:
	...
	def applyAction(self, motorCommands, terminated, terminated_task, stage, object_position_target):		
		#print(self.palmPos)
		#if(terminated_task == 0):
		...
		if (self.useInverseKinematics):
			...
			pinkyS = motorCommands[7]
				...
				#print(indexS)
			#print('positonX', positonX)
			ornX = motorCommands[9]
			ornY = motorCommands[8]
			ornZ = motorCommands[10]

			################################################### control arm ###################################################
			# surface -0.15
			#print("self.palmPos[2]: ", self.palmPos[2])
			if stage == 1:
				xbound = 0.05
				ybound = 0.05
				zbound = 0.12
				...
				if (self.palmPos[1] <= 0 - 0.05):
					self.palmPos[1] = 0 - 0.05
				...
				self.palmPos[1] = self.palmPos[1] + positonY
				if (self.palmPos[1] <= 0 - 0.03):
					self.palmPos[1] = 0 - 0.03
				if (self.palmPos[1] >= 0 + ybound):
					self.palmPos[1] = 0 + ybound
			...
			self.orn[0] = self.orn[0] + ornX
			if (self.orn[0] >= -0.15):
				self.orn[0] = -0.15
			if (self.orn[0] <= -0.85):
				self.orn[0] = -0.85

			self.orn[1] = self.orn[1] + ornY
			if (self.orn[1] <= -0.15):
				self.orn[1] = -0.15
			if (self.orn[1] >= 0.85):
				self.orn[1] = 0.85

			self.orn[2] = self.orn[2] + ornZ
			if (self.orn[2] >= -0.15):
				self.orn[2] = -0.15		
			...
			#orn = p.getQuaternionFromEuler([math.pi*(-0.5+ornX) , math.pi*(0+ornY), math.pi*(-0.5+ornZ)])
			orn = p.getQuaternionFromEuler([math.pi*(self.orn[0]) , math.pi*(self.orn[1]), math.pi*(self.orn[2])])
			...
			startPos = [self.handInitial[24], self.handInitial[25], self.handInitial[18], self.handInitial[19], self.handInitial[12], self.handInitial[13], self.handInitial[6], self.handInitial[7], self.handInitial[0], self.handInitial[1]] 	
			...
			if (terminated == 1):
				...
				posT = [self.palmPos[0], self.palmPos[1], (self.palmPos[2] + 0.2)]
				self.gt.palmP(posT, orn)
			#if(terminated_task == 1):
				#posT = [object_position_target[0], object_position_target[1], object_position_target[2]]
				#self.gt.palmP(posT, orn)	
