...
objectdir = os.path.join(startdir, "3dmodels/poPdAb24/")
import math
import time
import gym
...
class sawyerEnv(gym.Env):
	...
	def __init__(self, urdfRoot=pybullet_data.getDataPath(), 
		...
		if self._renders:
			...
			p.connect(p.DIRECT)
		self.handPoint = 43   # "palm point" for each grasp type 
		...
		lowerObservation = [-3000]*6 + [-1]* 42
		upperObservation = [3000]*6 + [1]* 42
	...
	def getExtendedObservation(self):
		...
		self._observation = norm + dist + obHand + obThumb + obIndex + obMid + obRing + [self.objectFeature[8]] + [p.getClosestPoints(self._sawyer.sawyerId, self.objectId, 500, self.handPoint, -1)[0][8]] + [self.objectFeature[2], self.objectFeature[3], self.objectFeature[4]] + [obPos[2]]
		return np.array(self._observation)
	...
	def _termination(self, action):	
		...
		if(contactParts[1] and contactParts[2] and contactParts[3] and self.inPosition()): # todo: decide z coordinates
			self.terminated = 1
			for i in range(200):
				self._sawyer.applyAction(action, self.terminated)
				p.stepSimulation()
	...
	def xInRange(self):
		...
		thumbTip = p.getLinkState(self._sawyer.sawyerId, 62)[0]
		indexTip = p.getLinkState(self._sawyer.sawyerId, 51)[0]
		return (obPos[0] + self.objectFeature[4]*0.5 + 0.002 < indexTip[0]) and (obPos[0] - self.objectFeature[4]*0.5 - 0.002> thumbTip[0])
	
	def yInRange(self):
		obPos, _ = p.getBasePositionAndOrientation(self.objectId)
		palmTip = p.getLinkState(self._sawyer.sawyerId, self.handPoint)[0]
		#return (palmTip[1] < obPos[1] + self.objectFeature[2]*0.25) and (palmTip[1] > obPos[1] - self.objectFeature[2]*0.25) 
		return (palmTip[1] < obPos[1] + 0.025) and (palmTip[1] > obPos[1] - 0.025) 
	'''
	def yInRange(self):
		obPos, _ = p.getBasePositionAndOrientation(self.objectId)
		indexTip = p.getLinkState(self._sawyer.sawyerId, 51)[0]
