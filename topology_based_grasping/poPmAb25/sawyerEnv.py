...
objectdir = os.path.join(startdir, "3dmodels/poPmAb25/")
import math
import time
import gym
...
class sawyerEnv(gym.Env):
	...
	def __init__(self, urdfRoot=pybullet_data.getDataPath(), 
		...
		lowerObservation = [-3000]*6 + [-1]* 48
		upperObservation = [3000]*6 + [1]* 48
	...
	def getExtendedObservation(self):
		...
		self._observation = norm + dist + obHand + obThumb + obIndex + obMid + obRing + obPinky + [self.objectFeature[8]] + [p.getClosestPoints(self._sawyer.sawyerId, self.objectId, 500, self.handPoint, -1)[0][8]] + [self.objectFeature[2], self.objectFeature[3], self.objectFeature[4]] + [obPos[2]]
		return np.array(self._observation)
	...
	def xInRange(self):
		...
		thumbTip = p.getLinkState(self._sawyer.sawyerId, 62)[0]
		indexTip = p.getLinkState(self._sawyer.sawyerId, 51)[0]
		return (obPos[0] + self.objectFeature[4]*0.5 + 0.001 < indexTip[0]) and (obPos[0] - self.objectFeature[4]*0.5 - 0.001> thumbTip[0])
